...
export const DamagePhotoUploader: React.FC<DamagePhotoUploaderProps> = ({
  ...
  return (
    <div className="space-y-4">
      ...
      <div
        ...
        className={`border-2 border-dashed rounded-3xl p-8 text-center cursor-pointer transition-all ${
          dragOver
            ...
            : 'border-slate-300 dark:border-slate-700 hover:border-teal-500 bg-slate-50/50 dark:bg-slate-900/40 hover:bg-teal-50/30 dark:hover:bg-teal-950/10'
        }`}
      >
        <input
          ref={fileInputRef}
        ...
        <div className="flex flex-col items-center justify-center gap-3">
          <div className="w-12 h-12 rounded-2xl bg-teal-50 dark:bg-teal-950/40 text-teal-600 dark:text-teal-400 shadow-sm border border-teal-200 dark:border-teal-800 flex items-center justify-center">
      ...
      {error && (
        <div className="p-3 bg-rose-500/10 rounded-xl border border-rose-500/20 text-xs text-rose-500 dark:text-rose-400 flex items-center gap-2">
      ...
      {items.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
          {items.map((item) => (
            <div
              key={item.id}
              className="relative group rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-2.5 flex flex-col items-center text-center overflow-hidden shadow-sm"
            >
              {item.type === 'image' ? (
                <img
                  src={item.previewUrl}
                ...
                <div className="w-full h-24 bg-slate-100 dark:bg-slate-800 rounded-xl flex items-center justify-center mb-2 text-slate-400">
                  {item.type === 'pdf' ? (
              ...
              <span className="text-[11px] font-medium text-slate-800 dark:text-slate-200 truncate w-full px-1">
                {item.file.name}
              </span>
