    ...
    <div className={`space-y-1.5 ${className}`}>
      ...
      <div className="relative">
        <input
          ...
          className={`w-full px-4 py-2.5 bg-slate-900/90 border rounded-xl font-mono text-sm tracking-widest text-white placeholder-slate-500 focus:outline-none focus:ring-2 transition-all ${
            !showValidation
              ? 'border-slate-700/80 focus:ring-blue-500/50 focus:border-blue-500'
              : feedback.isValid
              ? 'border-emerald-500/80 focus:ring-emerald-500/40 text-emerald-300'
        ...
        <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1.5 pointer-events-none">
          {showValidation && (
            feedback.isValid ? (
              <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-[11px] font-sans font-medium">
                <CheckCircle2 className="w-3.5 h-3.5" />
                <span>Provjeren</span>
              </span>
            ) : (
              <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-red-500/10 border border-red-500/30 text-red-400 text-[11px] font-sans font-medium">
      ...
      {showValidation ? (
        <p
          className={`text-[11px] font-medium flex items-center gap-1 ${
            feedback.isValid ? 'text-emerald-400' : 'text-red-400'
          }`}
        >
          {feedback.message}
        </p>
